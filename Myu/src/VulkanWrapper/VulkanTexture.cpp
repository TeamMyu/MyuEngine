...
namespace Myu::VulkanWrapper
{
    ...
    void VulkanTexture::createTextureTarget(VulkanDevice *device, uint32_t w, uint32_t h, VkFormat format, VkImageAspectFlagBits flag)
    {
        this->mDevice = device;
        this->width   = w;
        this->height  = h;
        ...
        Utils::transitionImageLayout(*mDevice, mImage, format, VK_IMAGE_LAYOUT_UNDEFINED, mSpec.imageLayout, flag);
        Utils::createImageView(device->GetVkLogicalDevice(), mImage, &mImageView, format, flag);
    ...
    void VulkanTexture::loadFromFile(VulkanDevice *device, const char *filePath)
    {
        ...
        createImage(mDevice->GetVkPhysicalDevice(), mDevice->GetVkLogicalDevice(), texWidth, texHeight, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_OPTIMAL, mSpec.imageUsageFlags, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, mImage, mImageMemory);
